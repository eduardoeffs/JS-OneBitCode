export default class {
    ...
    turnOn() {
        let currentCargeChecking = this.checkCurrentCharge()
        let shieldChecking = this.testShield()
        Promise.all([currentCargeChecking, shieldChecking]).then(results => {
            return this.shieldNormalizer(results[1])
        }).then(newShield => {
            this.spaceship.shield = newShield
            console.log("("+ this.spaceship.name +") " + "Partida autorizada: Escudo (" + this.spaceship.shield + ") / Carga em " + this.spaceship.currentCharge + "GJ")
        })
        .catch(error => {
    ...
    checkCurrentCharge() {
        return new Promise((resolve, reject) => {
            let currentCharge = this.spaceship.percentCurrentCharge()
            if(currentCharge >= 30) {
